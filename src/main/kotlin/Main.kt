...
fun minimumOf(number1: Long, number2: Long){
    ...
    if(number1 < number2) {
        ...
        println("Number 1 is greater than or equal to number 2!")
    }
}
...
fun inferredTypeOf(variable: Any){
    ...
    when(variable) {
        ...
        is Long -> println("$variable is an Long")
        is Double -> println("$variable is an Double")
        is Float -> println("$variable is an Float")
        is Boolean -> println("$variable is an Boolean")
        else -> println("The data type is something else")
    }
}
...
fun stringContains(str : String) = when(str){
    ...
    else -> println("$str doesn't contain the letters A, B or C")
